...
const inputClasses = "w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-slate-900 text-sm placeholder-slate-400 focus:ring-2 focus:ring-[var(--accent)]/30 focus:border-[var(--accent)] focus:outline-none transition-all";
                            ...
                            <div className="grid grid-cols-2 gap-3">
                                <button
                                    ...
                                    className={`p-4 rounded-xl border text-left transition-all cursor-pointer ${deliveryMethod === 'local'
                                        ...
                                        : 'border-[var(--border)] hover:border-[var(--border-hover)] hover:bg-white/[0.04]'
                                        }`}
                                >
                            ...
                            <div>
                                ...
                                <div className="space-y-2">
                                    {paymentMethods.map(method => (
                                        ...
                                        >
                                            <span className="flex items-center justify-center h-7 w-7 rounded-lg bg-white/[0.08] mr-3 text-sm font-bold text-[var(--text-secondary)]">{method.icon}</span>
            ...
            {showModal && (
                <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center z-50 p-4">
                    <div className={`bg-white rounded-2xl border shadow-xl p-8 max-w-sm w-full ${orderStatus === 'success' ? 'border-emerald-200' :
                        orderStatus === 'error' ? 'border-red-200' : 'border-[var(--accent)]/30'
                        } animate-fade-up`}>
                        <h3 className={`text-xl font-bold mb-3 text-center ${orderStatus === 'success' ? 'text-emerald-600' :
                            orderStatus === 'error' ? 'text-red-500' : 'text-[var(--accent)]'
                            }`}>
