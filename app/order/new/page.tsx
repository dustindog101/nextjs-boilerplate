...
interface FormInputProps { label: string; name: string; value: string; onChange: (e: React.ChangeEvent<HTMLInputElement>) => void; placeholder?: string; type?: string; }
interface FormSelectProps { label?: string; name: string; value: string; onChange: (e: React.ChangeEvent<HTMLSelectElement>) => void; options: string[]; }
interface FileInputProps { label: string; name: string; onChange: (e: React.ChangeEvent<HTMLInputElement>) => void; fileName?: string; }
interface IdFormProps { formData: IdFormData; onChange: (id: number, e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement>, isFile?: boolean) => void; }
...
// --- Components ---
const FormInput: React.FC<FormInputProps> = ({ label, name, value, onChange, placeholder = '', type = 'text' }) => (<div><label htmlFor={name} className="block text-sm font-medium text-gray-400 mb-1">{label}</label><input type={type} id={name} name={name} value={value} onChange={onChange} placeholder={placeholder} className="w-full bg-gray-700 border border-gray-600 rounded-lg px-3 py-2 text-white focus:ring-2 focus:ring-blue-500" /></div>);
const FormSelect: React.FC<FormSelectProps> = ({ label, name, value, onChange, options }) => (<div>{label && <label htmlFor={name} className="block text-sm font-medium text-gray-400 mb-1">{label}</label>}<select id={name} name={name} value={value} onChange={onChange} aria-label={label || name} className="w-full bg-gray-700 border border-gray-600 rounded-lg px-3 py-2 text-white focus:ring-2 focus:ring-blue-500">{options.map(opt => <option key={opt} value={opt}>{opt}</option>)}</select></div>);
const FileInput: React.FC<FileInputProps> = ({ label, name, onChange, fileName }) => (<div><label className="block text-sm font-medium text-gray-400 mb-1">{label}</label><div className="flex items-center justify-center w-full"><label htmlFor={name} className="flex flex-col items-center justify-center w-full h-32 border-2 border-gray-600 border-dashed rounded-lg cursor-pointer bg-gray-700/50 hover:bg-gray-700"><div className="flex flex-col items-center justify-center pt-5 pb-6"><UploadIcon /><p className="mb-2 text-sm text-gray-400"><span className="font-semibold">Click to upload</span></p>{fileName ? (<p className="text-xs text-green-400">{fileName}</p>) : (<p className="text-xs text-gray-500">PNG, JPG, or GIF</p>)}</div><input id={name} name={name} type="file" className="hidden" onChange={onChange} /></label></div></div>);
...
function OrderFormPage() {
    ...
    return (
        <div className="relative flex h-full font-sans">
            <div onClick={() => setSidebarOpen(false)} className={`fixed inset-0 bg-black/60 z-30 md:hidden ${isSidebarOpen ? 'block' : 'hidden'}`}></div>
                ...
                <div className="h-full w-64 px-3 py-4 overflow-y-auto">
                    ...
                    <ul className="space-y-2 font-medium">
                        {idForms.map((form, index) => (
                            <li key={form.id}>
                                <button 
                                    onClick={() => {setActiveFormId(form.id); if (window.innerWidth < 768) setSidebarOpen(false);}} 
                ...
                <div className="p-4 sm:p-8">
                    ...
                    <header className="text-center mb-12">
                        ...
                        <p className="mt-2 text-lg text-gray-400">
                            Editing ID #{idForms.findIndex(f => f.id === activeFormId) + 1}
                        </p>
                    </header>
...
// --- END OF FILE app/order/new/page.tsx (Full, Corrected Code) ---