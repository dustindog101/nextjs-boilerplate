...
- `**withResellerAuth.tsx`** protects `/reseller` (reseller or admin).
- **Implemented (2026):** Reseller order APIs are **not** mixed into generic LOOKUP list/update. They live in a dedicated Lambda and routes:
  - **Repo:** `lambda functions/reseller_handler/lambda_function.py` → env `**RESELLER_LAMBDA_URL`**
  - **Next.js:** `GET /api/reseller/orders`, `GET /api/reseller/orders/[orderId]`, `POST /api/reseller/update-order`
  - **Client:** `fetchResellerOrders`, `fetchResellerOrderById`, `resellerUpdateOrder` in `lib/apiClient.ts`
- **Ownership:** Whitelabel orders store the **subdomain slug** in `order.userId` (and `resellerId`). The reseller JWT has `**username`** = that slug and `**userId**` = account UUID. The reseller Lambda scopes list/update using `**jwt.username**` (and optional merge by JWT `userId` for the reseller’s own retail orders). **Admins** bypass ownership checks in `reseller_handler` when `role === 'admin'`.
- **Admin panel** is unchanged: `POST /api/admin` → `admin_handler` (`admin_update_order`, `list_all_orders`, etc.).

### Data fields (current)

- Create-order Lambda persists `**source`**, `**resellerId**`, and `**userId**` from the client. White-label checkout sends `**resellerId**` explicitly from `app/r/[resellerId]/page.tsx` in addition to `**userId: resellerId**` (slug) for attribution and future analytics.