...
- Backend services: AWS Lambda handlers (Python)
...
8. Keep Lambda env vars server-only (`AUTH_LAMBDA_URL`, `ORDER_LAMBDA_URL`, `LOOKUP_LAMBDA_URL`, `ADMIN_LAMBDA_URL`); never use `NEXT_PUBLIC_`.
...
- Backend handler updates: `aws/handlers/`
...
1. `app/api/orders/track/route.ts` currently forces `requestType: summary` and may conflict with API client methods requiring other request types.
2. `app/hooks/useOrder.ts` still has partial mock/editing flow behavior and TODO comments.
3. `aws/` contains duplicate legacy lambda files outside `aws/handlers/`; verify source of truth before backend refactors.
...
2. `aws/handlers/*` is the intended modern backend source set.
